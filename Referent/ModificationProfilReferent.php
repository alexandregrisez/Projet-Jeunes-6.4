...
function modifdemande($id,$nom,$prenom,$metier,$nvemail){
    ...
    while($buffer!=false){
        ...
        if($buffer!=false){
            ...
            if(isset($tab[1])){
                if($id==$tab[1]){
                    $ok=0;
                    if(fwrite($temp,"lol\n")==false){ //réussir à écrire nvl données et tableau dans temp!
                        return 10;
                    }
                }
                else{
    ...
    if($ok==0){
        ...
        rename("temp.txt","../Jeune/demandes.txt");
        return 0;
    }
   else{
        unlink("temp.txt");
